...
if (missing(the_top) | is.null(the_top) | is.na(the_top)) {
  ...
  the_top=3
} else {the_top = as.integer(the_top)}
...
# Get top candidates files from directories
  ...
  if (the_top < 2) {
    count_type = "One"
  } else {count_type = "None"}
  
  fst_cat1_cat2<-all_add[V8=="FST" & V9==count_type,.N, by=.(V7,V15,V6,V14,V17,V18)]
  fst_cat1_cat2<-fst_cat1_cat2[order(V15)]
  fst_cat1_cat2$an<-"FST"
  
  jsfs_cat1_cat2<-all_add[V8=="JSFS" & V9==count_type,.N, by=.(V7,V15,V6,V14,V17,V18)]
  ...
  if ("iHS" %in% all_add$V8) {
    ihs_cat1_cat2<-all_add[V8=="iHS" & V9==count_type,.N, by=.(V7,V15,V6,V14,V17,V18)]
  ...
  if ("GWAS" %in% all_add$V8) {
    gwas_cat1_cat2<-all_add[V8=="GWAS" & V9==count_type,.N, by=.(V7,V15,V6,V14,V17,V18)]
    gwas_cat1_cat2<-gwas_cat1_cat2[order(V15)]
    gwas_cat1_cat2$an<-"GWAS"
    ans_cat1_cat2<-rbind(ans_cat1_cat2,gwas_cat1_cat2)
  } 
  if ("TD" %in% all_add$V8) {
    td_cat1_cat2<-all_add[V8=="TD" & V9==count_type,.N, by=.(V7,V15,V6,V14,V17,V18)]
    td_cat1_cat2<-td_cat1_cat2[order(V15)]
    td_cat1_cat2$an<-"TD"
    ans_cat1_cat2<-rbind(ans_cat1_cat2,td_cat1_cat2)
  }
  if ("PI" %in% all_add$V8) {
    pi_cat1_cat2<-all_add[V8=="PI" & V9==count_type,.N, by=.(V7,V15,V6,V14,V17,V18)]
    pi_cat1_cat2<-pi_cat1_cat2[order(V15)]
    pi_cat1_cat2$an<-"PI"
    ans_cat1_cat2<-rbind(ans_cat1_cat2,pi_cat1_cat2)
  }
  
  #nsims = nrow(all_add[V8=="FST",.N, by=.(V7,V15,V6,V14,V17,V18)])
  nsims = 1000
  if (the_top < 2) {
    mean_ans<-ans_cat1_cat2[, .(mean_sim = sum(N)/nsims), by =.(an,V17,V18,V15)]
  } else {
    ans_cat1_cat2[ans_cat1_cat2$N<the_top]$N=the_top-1
    mean_ans<-ans_cat1_cat2[, .(mean_sim = the_top-mean(N)), by =.(an,V17,V18,V15)]
    }
...
## Needs to output a table as a tab delimited output and a plotted image.  
  ...
  mean_ans$perc<-(mean_ans$mean_sim)*100
  ...
  write.table(power_table,file = paste0(model,"_top",the_top,"_power.txt"),quote = FALSE,sep = "\t",row.names = FALSE)
